...
execute_init()
{
	...
	sed -i "s/quickcourse-claude-test/${REPONAME}/g" *.* > /dev/null 2>&1
	sed -i "s/quickcourse-claude-test/${REPONAME}/g" supplemental-ui/partials/header-content.hbs > /dev/null 2>&1
	echo -e "done"
